...
public class CommonService implements ICommonService {
	...
	public boolean isAuthUser(long rchId, String tokenId) {
		...
		if (rchId > 0 && tokenId != null && !tokenId.trim().isEmpty() && authUserRepository.findAll().stream()
				.anyMatch(user -> user.getUserId() == rchId && user.getTokenId().equals(tokenId))) {
		...
		return CompletableFuture.supplyAsync(() -> {
			...
			try {
				...
				ResponseEntity<String> response = restTemplate.exchange(API_URL, HttpMethod.POST,
						new HttpEntity<>(jsonRequestBody, createHeaders(API_KEY)), String.class);
